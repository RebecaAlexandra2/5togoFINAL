const pozeProduse = {
  ...
  function adaugaInCos(id) {
    ...
    if (user && user.role === "admin") {
      alert("Adminul nu poate adăuga produse în coș.");
      return;
    }
    const idx = cos.findIndex(item => item.id === id);
    if (idx !== -1) {
  ...
  async function updatePuncteFidelitate() {
    ...
    try {
      ...
      if (user.role !== "admin") {
        ...
        document.getElementById("user-puncte").textContent = "";
      }
    } catch (err) {}
  }
  ...
  function ascundeButoaneAdmin() {
    const user = JSON.parse(localStorage.getItem("user"));
    if (user && user.role === "admin") {
      // Ascunde butoanele „Adaugă în coș” generate deja în DOM
      ...
      if (mesajContainer) {
        ...
        mesajContainer.style.color = "red";
        mesajContainer.style.fontWeight = "bold";
        mesajContainer.style.marginTop = "10px";
      }
    }
  }
  ...
  window.onload = async () => {
    ...
    try {
      ...
      // După ce meniul a fost generat, ascundem butoanele dacă e admin
      ascundeButoaneAdmin();
      ...
      document.getElementById("finalizare-comanda").onclick = async () => {
        ...
        try {
          ...
          alert("Eroare server la plasarea comenzii.");
        }
      };
    } catch (err) {
